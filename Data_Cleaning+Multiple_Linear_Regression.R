...
crashes_data <- data.frame(Month = as.integer(crashes_data_tidy1$Month), Day = as.integer(crashes_data_tidy1$Day), CRASH.TIME = crashes_data_tidy1$CRASH.TIME, PRECINCT = crashes_data_tidy1$PRECINCT, TOTAL.COLLISION = crashes_data_tidy1$TOTAL.COLLISION)
...
model <- lm((TOTAL.COLLISION ~ Month + Day + PRECINCT),crashes_data_train)
summary(model)